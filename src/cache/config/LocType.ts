...
export default class LocType extends ConfigType {
    ...
    private static multilocBaseVars: Set<number> = new Set();
    private static multilocBaseVarsReady = false;
    ...
    static parse(server: Packet, jag: Jagfile) {
        ...
        LocType.configs = [];
        LocType.multilocBaseVars = new Set();
        LocType.multilocBaseVarsReady = false;
    ...
    static dependsOnVar(id: number): boolean {
        if (!this.multilocBaseVarsReady) {
            this.rebuildMultilocBaseVars();
        }

        return this.multilocBaseVars.has(id);
    }

    private static rebuildMultilocBaseVars(): void {
        this.multilocBaseVars.clear();

        for (const config of this.configs) {
            if (!config || !config.multiloc || config.multiloc.length === 0) {
                continue;
            }

            if (config.multivarbit !== -1) {
                const varbit = VarBitType.get(config.multivarbit);
                if (varbit) {
                    this.multilocBaseVars.add(varbit.basevar);
                }
            } else if (config.multivarp !== -1) {
                this.multilocBaseVars.add(config.multivarp);
            }
        }

        this.multilocBaseVarsReady = true;
    }
    ...
    getMultiLocId(getVarBit: (id: number) => number, getVar: (id: number) => number): number {
        if (!this.multiloc || this.multiloc.length === 0) {
            return this.id;
        }

        let value = -1;
        if (this.multivarbit !== -1) {
            value = getVarBit(this.multivarbit);
        } else if (this.multivarp !== -1) {
            value = getVar(this.multivarp);
        }

        if (value >= 0 && value < this.multiloc.length - 1) {
            return this.multiloc[value];
        }

        return this.multiloc[this.multiloc.length - 1];
    }
}
