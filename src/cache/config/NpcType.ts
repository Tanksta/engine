...
export default class NpcType extends ConfigType {
    ...
    getMultiNpcId(getVarBit: (id: number) => number, getVar: (id: number) => number): number {
        if (!this.multinpc || this.multinpc.length === 0) {
            return this.id;
        }
        ...
        if (value >= 0 && value < this.multinpc.length - 1) {
            return this.multinpc[value];
        }

        return this.multinpc[this.multinpc.length - 1];
    }
}
