    ...
    static lengths: number[] = [];
        ...
        this.lengths = new Array(count).fill(0);
	        ...
	        for (let i = 0; i < count; i++) {
	            if (!OnDemand.cache.has(3, i)) {
	                continue;
	            }
	
	            const packed = OnDemand.cache.read(3, i, false);
	            if (!packed) {
	                printWarning(`Failed to read midi id=${i}`);
	                continue;
	            }
            ...
            if (packed.length >= 2 && packed[0] === 0x1f && packed[1] === 0x8b) {
                try {
                    data = new Uint8Array(zlib.gunzipSync(packed));
                } catch {
                    printWarning(`Failed to decompress midi id=${i}`);
                    continue;
                }
            }
