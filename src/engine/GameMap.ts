...
export default class GameMap {
    ...
    private readonly instancedMapsquares: Set<string>;
    ...
    constructor(members: boolean) {
        ...
        this.instancedMapsquares = new Set();
    }
    ...
    createInstanceZone(zoneIndex: number): InstanceZone {
    ...
    ensureInstancedMapsquare(mx: number, mz: number, baseLevel: number): void {
        const key = `${mx}_${mz}_${baseLevel}`;
        if (this.instancedMapsquares.has(key)) {
            return;
        }

        const path = 'data/pack/server/maps/';
        const mapPath = `${path}m${mx}_${mz}`;
        const locPath = `${path}l${mx}_${mz}`;
        const objPath = `${path}o${mx}_${mz}`;

        if (!fs.existsSync(mapPath) || !fs.existsSync(locPath)) {
            throw new Error(`Cannot instance missing mapsquare m${mx}_${mz}.`);
        }
        ...
        this.loadGroundIntoLevels(lands, Packet.load(mapPath), mapsquareX, mapsquareZ, baseLevel);
        this.loadLocationsIntoLevels(lands, Packet.load(locPath), mapsquareX, mapsquareZ, baseLevel);

        if (fs.existsSync(objPath)) {
            this.loadObjsIntoLevels(Packet.load(objPath), mapsquareX, mapsquareZ, baseLevel);
        }

        this.instancedMapsquares.add(key);
    }
    ...
    private loadObjsIntoLevels(packet: Packet, mapsquareX: number, mapsquareZ: number, levelOffset: number): void {
        while (packet.available > 0) {
            const { x, z, level } = this.unpackCoord(packet.g2());
            const absoluteX: number = mapsquareX + x;
            const absoluteZ: number = mapsquareZ + z;
            ...
            for (let index: number = 0; index < count; index++) {
                ...
                const objType: ObjType = ObjType.get(id);
                if ((objType.members && this.members) || !objType.members) {
                    const obj: Obj = new Obj(levelOffset + level, absoluteX, absoluteZ, EntityLifeCycle.RESPAWN, objType.id, count);
                    this.getZone(obj.x, obj.z, obj.level).addStaticObj(obj);
                }
            }
        }
    ...
    private loadGroundIntoLevels(lands: Int8Array, packet: Packet, mapsquareX: number, mapsquareZ: number, levelOffset: number): void {
        for (let level: number = 0; level < GameMap.Y; level++) {
            for (let x: number = 0; x < GameMap.X; x++) {
                for (let z: number = 0; z < GameMap.Z; z++) {
                    while (true) {
        ...
        for (let level: number = 0; level < GameMap.Y; level++) {
            for (let x: number = 0; x < GameMap.X; x++) {
                ...
                for (let z: number = 0; z < GameMap.Z; z++) {
                    ...
                    if (x % 7 === 0 && z % 7 === 0) {
                        rsmod.allocateIfAbsent(absoluteX, absoluteZ, levelOffset + level);
                    }

                    const land: number = lands[this.packCoord(x, z, level)];

                    if ((land & GameMap.REMOVE_ROOFS) !== GameMap.OPEN) {
                        changeRoofCollision(absoluteX, absoluteZ, levelOffset + level, true);
                    }
                    ...
                    changeLandCollision(absoluteX, absoluteZ, levelOffset + actualLevel, true);
                }
            }
        }
    }
    ...
    private loadLocationsIntoLevels(lands: Int8Array, packet: Packet, mapsquareX: number, mapsquareZ: number, levelOffset: number): void {
        let locId: number = -1;
        let locIdOffset: number = packet.gsmarts();
        while (locIdOffset !== 0) {
            locId += locIdOffset;
            ...
            while (coordOffset !== 0) {
                ...
                const shape: number = info >> 2;
                const angle: number = info & 0x3;
                const instanceLevel = levelOffset + actualLevel;

                if (type.blockwalk) {
                    changeLocCollision(shape, angle, type.blockrange, length, width, type.active, absoluteX, absoluteZ, instanceLevel, true);
                }

                this.getZone(absoluteX, absoluteZ, instanceLevel).addStaticLoc(new Loc(instanceLevel, absoluteX, absoluteZ, width, length, EntityLifeCycle.RESPAWN, locId, shape, angle));
            }
            locIdOffset = packet.gsmarts();
        }
    }
    ...
    private loadCsvMap(map: Set<number>, csv: string[]): void {
        ...
        for (let index: number = 0; index < csv.length; index++) {
            ...
            const range: string[] = line.split(',');
            if (range.length === 2) {
                const [fromLevel, fromMx, fromMz, fromLx, fromLz] = range[0].split('_').map(Number);
                const [_toLevel, toMx, toMz, toLx, toLz] = range[1].split('_').map(Number);
                ...
                for (let x: number = startX; x <= endX; x += 8) {
                    for (let z: number = startZ; z <= endZ; z += 8) {
                        map.add(ZoneMap.zoneIndex(x, z, fromLevel));
                    }
                }
                continue;
            }
...
export function allocateZoneIfAbsent(level: number, x: number, z: number): void {
    rsmod.allocateIfAbsent(x, z, level);
}

export function deallocateZoneIfPresent(level: number, x: number, z: number): void {
