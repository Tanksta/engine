...
import { NpcInfoProt, PlayerInfoProt } from '#/network/rsbuf/index.js';
...
import { PetAcquisitionEvent } from '#/engine/entity/tracking/PetAcquisition.js';
...
import { isProfanityFilterEnabled } from '#/engine/PlayerSettings.js';
import ScriptPointer from '#/engine/script/ScriptPointer.js';
import ScriptProvider from '#/engine/script/ScriptProvider.js';
import ScriptRunner from '#/engine/script/ScriptRunner.js';
import ScriptState from '#/engine/script/ScriptState.js';
import { resolveTzHaarFightCaveView } from '#/engine/TzHaarFightCave.js';
...
import { isHardcodedBannedLoginIp, normalizeRemoteAddress, SYSTEM_ROUTER_BANNED_RESPONSE } from '#/server/login/HardcodedLoginBans.js';
...
import { MidiNames } from '#/cache/midi/MidiNames.js';
...
class World {
    ...
    private static readonly ADMIN_STAFF_MOD_LEVEL: number = 4;
    ...
    private static readonly WORLD_CLOCK_SAVERATE: number = 100; // 1m world clock checkpoint

    private static readonly AFK_EVENTRATE: number = 500; // 5m: 60/5 = 12 chances per hour
    private static readonly AFK_CHANCE1: number = 1.5 / (135 / 5); // 1/18 - 5.6% chance every 5 mins: avg 1 event every 90 mins
    private static readonly AFK_CHANCE2: number = 1.5 / (135 / 5); // same rate in aggro areas
    private static readonly AFK_SKILL_CHANCE1: number = 1.5 / (135 / 5); // 1/18 - 5.6% chance every 5 mins: avg 1 skilling event every 90 mins
    private static readonly AFK_SKILL_CHANCE2: number = 1.5 / (135 / 5); // same rate in aggro areas
    ...
    private static readonly TIMEOUT_NO_RESPONSE: number = Environment.NODE_DEBUG_SOCKET ? 60000 : 150; // 90s without any response
    ...
    npcKillCountGroup: Map<string, NpcKillCountEvent> = new Map();
    petAcquisitionEvents: PetAcquisitionEvent[] = [];
    wealthTransactionGroup: Map<string, WealthTransactionEvent> = new Map();
    wealthTransactions: WealthTransactionEvent[] = [];
    trackedNpcKillScripts: Set<string> = new Set();
    ...
    reload(clearInvs: boolean = true): void {
        ...
        this.loadTrackedNpcKillScripts();
        if (Environment.NODE_DEBUG) {
            if (count === -1) {
                this.broadcastAdminMes('There was an issue while reloading scripts.');
            } else {
                this.broadcastAdminMes(`Loaded ${count} scripts.`);
            }
        } else {
            if (count === -1) {
    ...
    cycle(): void {
        try {
            ...
            if (tick % World.WORLD_CLOCK_SAVERATE === 0 && tick > 0) {
                this.saveWorldClock();
            }
            ...
            if (this.npcKillCountGroup.size > 0) {
                this.loggerThread.postMessage({
                    type: 'npc_killcount',
                    events: Array.from(this.npcKillCountGroup.values())
                });

                this.npcKillCountGroup.clear();
            }

            if (this.petAcquisitionEvents.length > 0) {
                this.loggerThread.postMessage({
                    type: 'pet_acquisition',
                    events: this.petAcquisitionEvents
                });

                this.petAcquisitionEvents = [];
            }
        ...
        for (const player of this.playerLoop.all()) {
            try {
                ...
                if (this.currentTick % World.AFK_EVENTRATE === 0) {
                    ...
                    player.afkSkillingEventReady = Math.random() < (player.zonesAfk() ? World.AFK_SKILL_CHANCE2 : World.AFK_SKILL_CHANCE1);
                }
            ...
            if (player.loggingOut && (force || this.currentTick >= player.preventLogoutUntil)) {
                ...
                for (const request of player.queue.all()) {
	                ...
	                if (player.canAccess() && player.engineQueue.head() === null && queueDiscardable) {
	                    ...
	                    if (!script) {
	                        printWarning('Logout trigger unavailable; removing player without logout script.');
	                        this.removePlayer(player);
	                        continue;
	                    }
    ...
    private processLogins(): void {
        ...
        player: for (const player of this.newPlayers) {
            ...
            if (this.hasOtherPlayerFromAddress(player)) {
                if (isClientConnected(player)) {
                    player.addSessionLog(LoggerEventType.ENGINE, 'Tried to log in - address already has a player online');
                    this.forceLogout(player, 9);
                }
            ...
            player.onLogin();
            void this.loadPlayerNpcKillCounts(player);
    ...
    private processInfo(): void {
        const offsetTile = (value: number, offset: number): number => value === -1 ? -1 : value + offset;
        const offsetFine = (value: number, offset: number): number => value === -1 ? -1 : value + offset;

        // TODO: benchmark this?
        for (const player of this.playerLoop.all()) {
            player.buildArea.rebuildNormal(); // set origin before compute player is why this is above.

            const view = resolveTzHaarFightCaveView(player.level, player.x, player.z);
            ...
            rsbuf.computePlayer({
                ...
                level: view.level,
                z: view.z,
                originX: offsetTile(player.originX, view.tileXOffset),
                originZ: offsetTile(player.originZ, view.tileZOffset),
                pid: player.slot,
                tele: player.tele,
                jump: player.jump,
                runDir: player.runDir,
                walkDir: player.walkDir,
                visibility: player.visibility,
                active: player.isActive,
                masks: player.masks,
                appearance,
                lastAppearance: player.lastAppearance,
                faceEntity: player.faceEntity,
                faceX: offsetFine(player.faceSquareX, view.fineXOffset),
                faceZ: offsetFine(player.faceSquareZ, view.fineZOffset),
                orientationX: offsetFine(player.faceAngleX, view.fineXOffset),
                orientationZ: offsetFine(player.faceAngleZ, view.fineZOffset),
                damageTaken: player.hitmarkDamage,
                damageType: player.hitmarkType,
                damageTaken2: player.hitmark2Damage,
                damageType2: player.hitmark2Type,
                currentHitpoints: player.levels[PlayerStat.HITPOINTS],
                baseHitpoints: player.baseLevels[PlayerStat.HITPOINTS],
                animId: player.animId,
                animDelay: player.animDelay,
                say: player.sayMessage,
                message: player.chatMessage,
                color: player.chatColour ?? -1,
                effect: player.chatEffect ?? -1,
                ignored: player.chatRights ?? 0,
                graphicId: player.spotanimId,
                graphicHeight: player.spotanimHeight,
                graphicDelay: player.spotanimTime,
                exactStartX: offsetTile(player.exactStartX, view.tileXOffset),
                exactStartZ: offsetTile(player.exactStartZ, view.tileZOffset),
                exactEndX: offsetTile(player.exactEndX, view.tileXOffset),
                exactEndZ: offsetTile(player.exactEndZ, view.tileZOffset),
                exactMoveStart: player.exactMoveStart,
                exactMoveEnd: player.exactMoveEnd,
                exactMoveDirection: player.exactMoveFacing
            });
        }

        for (const npc of this.npcs) {
            const view = resolveTzHaarFightCaveView(npc.level, npc.x, npc.z);
            rsbuf.computeNpc({
                x: view.x,
                level: view.level,
                z: view.z,
                ...
                faceX: offsetFine(npc.faceSquareX, view.fineXOffset),
                faceZ: offsetFine(npc.faceSquareZ, view.fineZOffset),
                orientationX: offsetFine(npc.faceAngleX, view.fineXOffset),
                orientationZ: offsetFine(npc.faceAngleZ, view.fineZOffset),
                damageTaken: npc.hitmarkDamage,
                damageType: npc.hitmarkType,
                damageTaken2: npc.hitmark2Damage,
                damageType2: npc.hitmark2Type,
                currentHitpoints: npc.levels[NpcStat.HITPOINTS],
                baseHitpoints: npc.baseLevels[NpcStat.HITPOINTS],
                animId: npc.animId,
                animDelay: npc.animDelay,
                say: npc.sayMessage,
                graphicId: npc.spotanimId,
                graphicHeight: npc.spotanimHeight,
                graphicDelay: npc.spotanimTime
            });
        }
    }

    private packPublicChatForPlayer(player: Player, _observer: Player): Uint8Array | null {
        if (player.chatRawMessage === null) {
            return player.chatMessage;
        }

        const out = Packet.alloc(0);
        WordPack.pack(out, player.chatRawMessage);
        const message = new Uint8Array(out.pos);
        out.pos = 0;
        out.gdata(message, 0, message.length);
        out.release();
        return message;
    }

    computePlayerInfoForPlayer(observer: NetworkPlayer): void {
        const offsetTile = (value: number, offset: number): number => value === -1 ? -1 : value + offset;
        const offsetFine = (value: number, offset: number): number => value === -1 ? -1 : value + offset;

        for (const player of this.playerLoop.all()) {
            const view = resolveTzHaarFightCaveView(player.level, player.x, player.z);
            const appearance = player.appearanceBuf ?? player.generateAppearance();
            ...
            rsbuf.computePlayer({
                ...
                animDelay: player.animDelay,
                say: player.sayMessage,
                message: this.packPublicChatForPlayer(player, observer),
                color: player.chatColour ?? -1,
                effect: player.chatEffect ?? -1,
                ignored: player.chatRights ?? 0,
                graphicId: player.spotanimId,
    ...
    computeNpcInfoForPlayer(player: NetworkPlayer): void {
        const offsetFine = (value: number, offset: number): number => value === -1 ? -1 : value + offset;

        for (const npc of this.npcs) {
            const view = resolveTzHaarFightCaveView(npc.level, npc.x, npc.z);
            const baseType = NpcType.get(npc.type);
            const resolvedType = player.resolveNpcType(npc);
            const active = npc.isActive && resolvedType !== null;
            let masks = npc.masks;
            let type = npc.type;
            ...
            if (resolvedType) {
                ...
                if (
                    type > NPC_INFO_SPAWN_TYPE_MAX ||
                    type !== npc.type ||
                    baseType.multivarbit !== -1 ||
                    baseType.multivarp !== -1
                ) {
                    masks |= NpcInfoProt.CHANGE_TYPE;
                }
            }
    ...
    private autosavePlayer(player: Player): void {
        if (player.syntheticAccount) {
            return;
        }
        this.postPlayerAutosave(player, player.save());
    }

    private postPlayerAutosave(player: Player, save: Uint8Array): void {
        this.loginThread.postMessage({
    ...
    private savePlayers(): void {
        ...
        for (const player of this.playerLoop.all()) {
            this.autosavePlayer(player);
        }
    }

    private getWorldClockPath(): string {
        return `data/world/${Environment.NODE_PROFILE}/clock.txt`;
    }

    private loadWorldClock(): void {
        const path = this.getWorldClockPath();
        if (!fs.existsSync(path)) {
            return;
        }

        const tick = Number.parseInt(fs.readFileSync(path, 'utf8').trim(), 10);
        if (Number.isFinite(tick) && tick > this.currentTick) {
            this.currentTick = tick;
        }
    }

    private saveWorldClock(): void {
        const path = this.getWorldClockPath();
        ...
        fs.writeFileSync(path, `${Math.max(previous, this.currentTick)}\n`);
    }
    ...
    addNpc(npc: Npc, duration: number, firstSpawn: boolean = true): void {
        if (firstSpawn) {
            const initialType = npc.type > NPC_INFO_SPAWN_TYPE_MAX ? NPC_INFO_SPAWN_PLACEHOLDER_TYPE : npc.type;
            rsbuf.addNpc(npc.nid, initialType);
        ...
        // NPC additions use an 11-bit type field in the client protocol.
        // For high-id NPCs, add them as a safe placeholder and immediately
        // send a CHANGE_TYPE mask so the client swaps to the real id via g2.
        if (firstSpawn && npc.type > NPC_INFO_SPAWN_TYPE_MAX) {
            npc.masks |= NpcInfoProt.CHANGE_TYPE;
        }
    ...
    addObj(obj: Obj, receiver64: bigint, duration: number): void {
        ...
        const owner64 = obj.owner64 !== Obj.NO_RECEIVER ? obj.owner64 : receiver64;
        obj.owner64 = owner64;
        ...
        if (ObjType.get(obj.type).stackable && obj.lifecycle === EntityLifeCycle.DESPAWN) {
            ...
            if (existing && existing.lifecycle === EntityLifeCycle.DESPAWN && existing.owner64 === owner64) {
    ...
    removeHeadlessPlayer(player: Player): void {
        ...
        player.cleanup();
        player.isActive = false;
        player.addSessionLog(LoggerEventType.MODERATOR, 'Headless player removed');
    }
    ...
    sendPrivateMessage(player: Player, targetUsername37: bigint, message: string): void {
        ...
        this.friendThread.postMessage({
            ...
            username: player.username,
            staffLvl: this.chatRights(player),
    ...
    private hasOtherPlayerFromAddress(player: Player): boolean {
        if (!(player instanceof NetworkPlayer)) {
            return false;
        }

        const remoteIp = normalizeRemoteAddress(player.client.remoteAddress);
        if (remoteIp === null) {
            return false;
        }

        for (const other of this.playerLoop.all()) {
            if (other.username === player.username || !(other instanceof NetworkPlayer)) {
                continue;
            }

            if (normalizeRemoteAddress(other.client.remoteAddress) === remoteIp) {
                return true;
            }
        }
    ...
    private createDevThread() {
            ...
            try {
                if (msg.type === 'dev_reload') {
                    ...
                    if (msg.error) {
                        ...
                        this.broadcastAdminMes(msg.error.replaceAll(`${Environment.BUILD_SRC_DIR}/scripts/`, ''));
                        this.broadcastAdminMes('Check the console for more information.');
                    }
                } else if (msg.type === 'dev_progress') {
                    if (msg.broadcast) {
                        printDebug(msg.broadcast);

                        this.broadcastAdminMes(msg.broadcast);
                    } else if (msg.text) {
                        printInfo(msg.text);
                    }
                }
        ...
        this.devThread.on('exit', () => {
            try {
                ...
                this.broadcastAdminMes('Error while rebuilding - see console for more info.');
    ...
    broadcastMes(message: string): void {
        for (const player of this.playerLoop.all()) {
            this.sendBroadcastMes(player, message);
        }
    }

    broadcastAdminMes(message: string): void {
        for (const player of this.playerLoop.all()) {
            if (player.staffModLevel >= World.ADMIN_STAFF_MOD_LEVEL) {
                this.sendBroadcastMes(player, message);
            }
        }
    }

    private sendBroadcastMes(player: Player, message: string): void {
        if (message.includes('\n')) {
    ...
    onLoginMessage(msg: GenericLoginThreadResponse) {
        if (isPlayerLoginResponse(msg)) {
            ...
            if (reply === -1) {
                ...
                client.close();
                return;
            } else if (reply === SYSTEM_ROUTER_BANNED_RESPONSE) {
                client.send(Uint8Array.from([SYSTEM_ROUTER_BANNED_RESPONSE]));
                client.close();
                return;
            }
            ...
            try {
                ...
                player.accountId = msg.account_id;
    ...
    onFriendMessage(msg: FriendThreadMessage) {
        ...
        try {
            if (opcode === FriendsServerOpcodes.UPDATE_FRIENDLIST) {
                ...
                const chat = isProfanityFilterEnabled(player) ? WordEnc.filter(rawChat) : rawChat;
    ...
    onClientData(client: ClientSocket) {
        ...
        if (client.opcode === 14) {
            ...
            if (rev !== Environment.ENGINE_REVISION) {
                printWarning(`Login rejected as outdated: revision mismatch from ${client.remoteAddress}; got ${rev}, expected ${Environment.ENGINE_REVISION}`);
            ...
            const loginCrc = Packet.getcrc(crcs, 0, crcs.length) >>> 0;
            const expectedCrc = CrcBuffer32 >>> 0;
            if (loginCrc !== expectedCrc) {
                printWarning(`Login rejected as outdated: CRC mismatch from ${client.remoteAddress}; got ${loginCrc}, expected ${expectedCrc}`);
                client.send(Uint8Array.from([6]));
                client.close();
                return;
            }
            ...
            if (World.loginBuf.g1() !== 10) {
                printWarning(`Login rejected as outdated: RSA block mismatch from ${client.remoteAddress}`);
                // RSA error
            ...
            if (isHardcodedBannedLoginIp(client.remoteAddress)) {
                client.send(Uint8Array.from([SYSTEM_ROUTER_BANNED_RESPONSE]));
                client.close();
                return;
            }
    ...
    private async loadPlayerNpcKillCounts(player: Player): Promise<void> {
        if (player.accountId < 1) {
            player.npcKillCountsLoaded = true;
            return;
        }

        try {
            const rows = await db
                .selectFrom('npc_killcount')
                .select(['npc_type', 'npc_name', 'kills'])
                .where('account_id', '=', player.accountId)
                .where('profile', '=', Environment.NODE_PROFILE)
                .where('kills', '>', 0)
                .execute();

            const merged = new Map<number, { npcName: string; kills: number }>();
            for (const row of rows) {
                merged.set(row.npc_type, {
                    npcName: row.npc_name,
                    kills: row.kills
                });
            }

            for (const [npcType, extraKills] of player.npcKillCountsPending) {
                const existing = merged.get(npcType);
                if (existing) {
                    existing.kills += extraKills;
                } else {
                    const pending = player.npcKillCounts.get(npcType);
                    merged.set(npcType, {
                        npcName: pending?.npcName ?? NpcType.get(npcType).name ?? `Npc ${npcType}`,
                        kills: extraKills
                    });
                }
            }

            for (const [npcType, pending] of player.npcKillCounts) {
                if (!merged.has(npcType) && pending.kills > 0) {
                    merged.set(npcType, pending);
                }
            }

            if (this.getPlayerByUsername(player.username) !== player && !this.newPlayers.has(player)) {
                return;
            }

            player.npcKillCounts = merged;
            player.npcKillCountsPending.clear();
            player.npcKillCountsLoaded = true;
        } catch (err) {
            printError(`Failed to load NPC killcounts for ${player.username}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }

    addNpcKillCount(player: Player, npcType: number) {
        if (player.accountId < 1) {
            return;
        }

        const npcName = NpcType.get(npcType).name ?? `Npc ${npcType}`;
        const existingCount = player.npcKillCounts.get(npcType);
        if (existingCount) {
            existingCount.kills += 1;
            existingCount.npcName = npcName;
        } else {
            player.npcKillCounts.set(npcType, {
                npcName,
                kills: 1
            });
        }

        if (!player.npcKillCountsLoaded) {
            player.npcKillCountsPending.set(npcType, (player.npcKillCountsPending.get(npcType) ?? 0) + 1);
        }
        ...
        const existing = this.npcKillCountGroup.get(key);
        if (existing) {
            existing.kills++;
            existing.last_kill = Date.now();
            return;
        }

        this.npcKillCountGroup.set(key, {
            account_id: player.accountId,
            profile: Environment.NODE_PROFILE,
            npc_type: npcType,
            npc_name: npcName,
    ...
    addPetAcquisition(player: Player, petItem: number, sourceType: string, sourceDetail: string | null = null) {
        if (player.accountId < 1 || petItem < 0) {
            return;
        }

        const obj = ObjType.get(petItem);
        const petName = obj?.name ?? `Pet ${petItem}`;
        const petKey = obj?.debugname ?? `obj_${petItem}`;

        this.petAcquisitionEvents.push({
            account_id: player.accountId,
            profile: Environment.NODE_PROFILE,
            pet_item: petItem,
            pet_key: petKey,
            ...
            source_detail: sourceDetail && sourceDetail.length > 0 ? sourceDetail : null,
            acquired_at: Date.now()
        });
    }

    shouldTrackNpcKillScript(scriptName: string): boolean {
        return this.trackedNpcKillScripts.has(scriptName);
    }

    private loadTrackedNpcKillScripts(): void {
        this.trackedNpcKillScripts.clear();

        this.addTrackedNpcKillScriptsFromPath(`${Environment.BUILD_SRC_DIR}/scripts/drop tables/scripts`);
        this.addTrackedNpcKillScriptsFromPath(`${Environment.BUILD_SRC_DIR}/scripts/skill_slayer/scripts/npcs`);
        this.addTrackedNpcKillScriptsFromFile(`${Environment.BUILD_SRC_DIR}/scripts/areas/area_wilderness/scripts/king_black_dragon.rs2`);
        this.addTrackedNpcKillScriptsFromPath(`${Environment.BUILD_SRC_DIR}/scripts/minigames/game_barrows/scripts/brothers`);
    }

    private addTrackedNpcKillScriptsFromPath(path: string): void {
        if (!fs.existsSync(path)) {
            return;
        }
        ...
        for (const entry of fs.readdirSync(path, { withFileTypes: true })) {
            ...
            if (entry.isDirectory()) {
                this.addTrackedNpcKillScriptsFromPath(fullPath);
            } else if (entry.isFile() && entry.name.endsWith('.rs2')) {
                this.addTrackedNpcKillScriptsFromFile(fullPath);
            }
        }
    }

    private addTrackedNpcKillScriptsFromFile(path: string): void {
        if (!fs.existsSync(path)) {
            return;
        }

        const source = fs.readFileSync(path, 'utf8');
        const matches = source.matchAll(/\[ai_queue3,([^\]]+)\]/g);
        for (const match of matches) {
            const scriptName = match[1]?.trim();
            if (scriptName) {
                this.trackedNpcKillScripts.add(`[ai_queue3,${scriptName}]`);
            }
        }
    }
    ...
    setBetaHiscores(enabled: boolean) {
        printInfo(`Beta highscores ${enabled ? 'enabled' : 'disabled'} for profile ${Environment.NODE_PROFILE}`);
        this.loginThread.postMessage({
            type: 'beta_hiscore_mode',
            enabled
        });
    }
...
const NPC_INFO_SPAWN_TYPE_MAX = 8191;
const NPC_INFO_SPAWN_PLACEHOLDER_TYPE = 1; // man
