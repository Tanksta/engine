...
export class NetworkPlayer extends Player {
        ...
        if (packetType) {
            ...
            if (decoder) {
                ...
                const clearIdleLogout =
                    message.category === ClientGameProtCategory.USER_EVENT ||
                    packetType === ClientGameProt.EVENT_MOUSE_CLICK ||
                    packetType === ClientGameProt.EVENT_MOUSE_MOVE ||
                    packetType === ClientGameProt.EVENT_CAMERA_POSITION ||
                    packetType === ClientGameProt.EVENT_APPLET_FOCUS;

                if (success && clearIdleLogout) {
                    this.idleLogoutAt = -1;
                }
    ...
    updatePlayers() {
        World.computePlayerInfoForPlayer(this);
        this.write(new PlayerInfo(rsbuf.playerInfo({
            pos: this.client.out.pos,
            pid: this.slot,
            dx: Math.abs(this.lastTickX - this.x),
            dz: Math.abs(this.lastTickZ - this.z),
            rebuild: this.lastLevel !== this.level
        })));
    }

    updateNpcs() {
        World.computeNpcInfoForPlayer(this);
        this.write(new NpcInfo(rsbuf.npcInfo({
            pos: this.client.out.pos,
            pid: this.slot,
            dx: Math.abs(this.lastTickX - this.x),
            dz: Math.abs(this.lastTickZ - this.z),
