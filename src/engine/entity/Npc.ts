...
export default class Npc extends PathingEntity {
    ...
    resetEntity(respawn: boolean) {
        if (respawn) {
            ...
            // High-id NPCs are added with an 11-bit type on the base spawn path.
            // Keep re-sending CHANGE_TYPE so late observers (or clients that miss
            // the initial upgrade) can resolve the real full g2 npc id.
            if (this.type > 2047) {
    ...
    private processQueue() {
        ...
        for (const request of this.queue.all()) {
            ...
            if (!this.delayed && request.delay <= 0) {
                ...
                if (script) {
                    if (request.queueId === ServerTriggerType.AI_QUEUE3 && this.levels[NpcStat.HITPOINTS] === 0) {
                        this.tryTrackKillCount(script.name);
                    }
    ...
    private tryTrackKillCount(scriptName: string): void {
        if (this.killCountLogged || !World.shouldTrackNpcKillScript(scriptName)) {
            return;
        }

        const hash64 = this.heroPoints.findHero();
        if (hash64 === -1n) {
            return;
        }

        const player = World.getPlayerByHash64(hash64);
        if (!player || player.accountId < 1) {
            return;
        }

        World.addNpcKillCount(player, this.type);
        this.killCountLogged = true;
    }
