...
export default abstract class PathingEntity extends Entity {
    ...
    protected inApproachDistance(range: number, target: Entity): boolean {
        ...
        if (this instanceof Npc) {
            const ignoreLosParam = ParamType.getId('ignore_los');
            if (ignoreLosParam !== -1 && NpcType.get(this.type).params?.get(ignoreLosParam) === 1) {
                return CoordGrid.distanceTo(this, target) <= range;
            }
