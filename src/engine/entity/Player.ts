...
import { clearViewedBankOwner, getViewedBankInventory } from '#/engine/AdminBankView.js';
import { Inventory, InventoryListener } from '#/engine/Inventory.js';
import { resolveTzHaarFightCaveSaveCoord } from '#/engine/TzHaarFightCave.js';
import ScriptFile from '#/engine/script/ScriptFile.js';
import ScriptPointer from '#/engine/script/ScriptPointer.js';
import ScriptProvider from '#/engine/script/ScriptProvider.js';
import ScriptRunner from '#/engine/script/ScriptRunner.js';
import ScriptState from '#/engine/script/ScriptState.js';
import { sendMissingTriggerMessage } from '#/engine/script/MissingTriggerMessage.js';
...
const APPEARANCE_SLOT_COUNT = 12;
const DEFAULT_GAME_OPTION_VARPS: readonly [string, number][] = [
    ['option_brightness', 2],
    ['option_music', 2],
    ['option_sounds', 2]
];

function getDefaultGameOptionVarpIds(): Map<number, number> {
    const defaults = new Map<number, number>();
    for (const [name, value] of DEFAULT_GAME_OPTION_VARPS) {
        const varp = VarPlayerType.getByName(name);
        if (varp && varp.type !== ScriptVarType.STRING) {
            defaults.set(varp.id, value);
        }
    }
    return defaults;
}

function shouldSaveNumericVarp(id: number, value: number, defaultGameOptionVarpIds: ReadonlyMap<number, number>): boolean {
    return value !== 0 || defaultGameOptionVarpIds.has(id);
}
...
export default class Player extends PathingEntity {
    ...
    save() {
        ...
        const saveCoord = resolveTzHaarFightCaveSaveCoord(this.x, this.z, this.level);
        sav.p2(saveCoord.x);
        sav.p2(saveCoord.z);
        ...
        const defaultGameOptionVarpIds = getDefaultGameOptionVarpIds();

        let saved = 0;
        for (let id = 0; id < this.vars.length; id++) {
            const varp = VarPlayerType.get(id);
            if (varp.scope === VarPlayerType.SCOPE_PERM && varp.type !== ScriptVarType.STRING && shouldSaveNumericVarp(id, this.vars[id], defaultGameOptionVarpIds)) {
                saved++;
            }
        }
        sav.p2(saved);
        ...
        let savedStrings = 0;
        for (let id = 0; id < this.varsString.length; id++) {
            const varp = VarPlayerType.get(id);
            const value = this.varsString[id];
            if (varp.scope === VarPlayerType.SCOPE_PERM && varp.type === ScriptVarType.STRING && value && value.length > 0) {
                savedStrings++;
            }
        }
        sav.p2(savedStrings);
    ...
    run: number = 0;
    tempRun: number = 0;
    private temporaryMoveStrategy: MoveStrategy | null = null;
    private previousMoveStrategy: MoveStrategy | null = null;
    private temporaryFlyDestination: { readonly x: number; readonly z: number } | null = null;
    runenergy: number = 10000;
    lastRunEnergy: number = -1;
    runweight: number = 0;
    playtime: number = 0;
    stats: Int32Array = new Int32Array(PlayerStatEnabled.length);
    levels: Uint8Array = new Uint8Array(PlayerStatEnabled.length);
    vars: Int32Array;
    varsString: string[];
    ...
    accountId: number = -1;
    syntheticAccount: boolean = false;
    npcKillCounts: Map<number, { npcName: string; kills: number }> = new Map();
    npcKillCountsPending: Map<number, number> = new Map();
    npcKillCountsLoaded: boolean = false;
    input: InputTracking;
    ...
    headicons: number = 0;
    pkHeadicon: number = -1;
    prayerHeadicon: number = -1;
    baseLevels = new Uint8Array(PlayerStatEnabled.length);
    lastStats: Int32Array = new Int32Array(PlayerStatEnabled.length); // we track this so we know to flush stats only once a tick on changes
    lastLevels: Uint8Array = new Uint8Array(PlayerStatEnabled.length); // we track this so we know to flush stats only once a tick on changes
    originX: number = -1;
    originZ: number = -1;
    mapBuildPending: boolean = false;
    ...
    private loginModalId = -1;
    private lastLoginInfoPacket: LastLoginInfo | null = null;
    ...
    private readonly appearanceSlotOverrides: Map<number, number> = new Map();
    transmogNpc: number = -1;
    private ringOfStoneUnmorphing: boolean = false;
    ...
    chatRawMessage: string | null = null;
    ...
    constructor(username: string, username37: bigint, hash64: bigint) {
        ...
        for (const [id, value] of getDefaultGameOptionVarpIds()) {
            this.vars[id] = value;
        }
    }
    ...
    onLogin() {
        ...
        this.loginModalId = this.modalMain;
    ...
    onReconnect() {
        ...
        if (!this.requestModalClose && this.loginModalId !== -1 && this.modalMain === this.loginModalId && (this.modalState & ModalState.MAIN) !== ModalState.NONE) {
            const welcome = ScriptProvider.getByName('[proc,welcome_screen]');
            if (welcome) {
                this.executeScript(ScriptRunner.init(welcome, this));
                this.loginModalId = this.modalMain;
            }
    ...
    addPetAcquisition(petItem: number, sourceType: string, sourceDetail: string | null = null) {
        World.addPetAcquisition(this, petItem, sourceType, sourceDetail);
    }
    ...
    updateMovement(): boolean {
        ...
        if (this.shouldHoldTemporaryFlyMovement()) {
            return false;
        }
        ...
        this.queuePendingTemporaryFlyWaypoints();
        const moved = super.processMovement();
        if (!moved) {
            // todo: this is running every idle tick
            this.tempRun = 0;
            this.stopTemporaryMoveStrategy();
        } else if (!this.hasWaypoints()) {
            if (this.hasPendingTemporaryFlyDestination()) {
                this.queuePendingTemporaryFlyWaypoints();
            } else {
                this.stopTemporaryMoveStrategy();
            }
    ...
    setTemporaryMoveStrategy(strategy: MoveStrategy): void {
        if (this.temporaryMoveStrategy === null) {
            this.previousMoveStrategy = this.moveStrategy;
        }
        this.temporaryMoveStrategy = strategy;
    ...
    queueTemporaryFlyWaypoints(destinationX: number, destinationZ: number): void {
        this.temporaryFlyDestination = { x: destinationX, z: destinationZ };
        this.clearWaypoints();
        this.setTemporaryMoveStrategy(MoveStrategy.FLY);
        this.queuePendingTemporaryFlyWaypoints();
    }

    hasTemporaryMoveStrategy(): boolean {
        return this.temporaryMoveStrategy !== null || this.temporaryFlyDestination !== null;
    }

    private hasPendingTemporaryFlyDestination(): boolean {
        return this.temporaryFlyDestination !== null && (this.x !== this.temporaryFlyDestination.x || this.z !== this.temporaryFlyDestination.z);
    }

    private shouldHoldTemporaryFlyMovement(): boolean {
        return this.mapBuildPending && this.hasPendingTemporaryFlyDestination();
    }

    private queuePendingTemporaryFlyWaypoints(): void {
        if (this.temporaryFlyDestination === null || this.waypointIndex > 0) {
            return;
        }
        ...
        while (waypoints.length < this.waypoints.length && (x !== this.temporaryFlyDestination.x || z !== this.temporaryFlyDestination.z)) {
            const direction: number = CoordGrid.face(x, z, this.temporaryFlyDestination.x, this.temporaryFlyDestination.z);
            if (direction === -1) {
                break;
            }

            x = CoordGrid.moveX(x, direction);
            z = CoordGrid.moveZ(z, direction);
            waypoints.push(CoordGrid.packCoord(0, x, z));
        }
    ...
    private stopTemporaryMoveStrategy(): void {
        this.temporaryFlyDestination = null;
        this.restoreTemporaryMoveStrategy();
    }

    private restoreTemporaryMoveStrategy(): void {
        if (this.temporaryMoveStrategy === null) {
            return;
        }
        this.moveStrategy = this.previousMoveStrategy ?? MoveStrategy.SMART;
        this.temporaryMoveStrategy = null;
        this.previousMoveStrategy = null;
    }
    ...
    private hasRepeatingCombatInteraction(): boolean {
        return (
            (this.target instanceof Npc && this.targetOp === ServerTriggerType.APNPC2) ||
            (this.target instanceof Player && this.targetOp === ServerTriggerType.APPLAYER2)
        );
    }

    // clear current interaction but leave walk queue intact
    clearPendingAction(preserveCombatInteraction: boolean = false) {
        this.unmorphRingOfStone();

        if (!preserveCombatInteraction || !this.hasRepeatingCombatInteraction()) {
            this.clearInteraction();
        }
        this.closeModal();
    }

    unmorphRingOfStone(): void {
        if (this.ringOfStoneUnmorphing) {
            return;
        }

        if (this.transmogNpc !== NpcType.getId('tzhaar_morph_rock')) {
            return;
        }

        const script = ScriptProvider.getByName('[proc,ring_of_stone_unmorph]');
        if (!script) {
            return;
        }

        const wasProtected = this.protect;
        this.ringOfStoneUnmorphing = true;
        try {
            this.executeScript(ScriptRunner.init(script, this), true, true);
        } finally {
            this.ringOfStoneUnmorphing = false;
            this.protect = wasProtected;
        }
    }
    ...
    getOpTrigger() {
        ...
        const trigger = (this.targetOp + 7) as ServerTriggerType;
        if (this.target instanceof Loc && this.targetSubject.com === -1) {
            return this.getLocTrigger(trigger);
        }
        ...
        if (this.target instanceof Npc || this.target instanceof Loc || this.target instanceof Obj) {
            ...
            if (this.target instanceof Npc) {
                type = this.resolveNpcType(this.target);
            } else if (this.target instanceof Loc) {
                type = this.resolveLocType(this.target);
        ...
        return ScriptProvider.getByTrigger(trigger, typeId, categoryId) ?? null;
    }
    ...
    getApTrigger() {
        ...
        const trigger = this.targetOp as ServerTriggerType;
        if (this.target instanceof Loc && this.targetSubject.com === -1) {
            return this.getLocTrigger(trigger);
        }
    ...
    getLocTrigger(trigger: ServerTriggerType): ScriptFile | null {
        if (!(this.target instanceof Loc)) {
            return null;
        }

        const baseType = LocType.get(this.target.type);
        if (!baseType) {
            return null;
        }

        const resolvedType = this.resolveLocType(this.target);
        if (resolvedType && resolvedType.id !== baseType.id) {
            let script = ScriptProvider.getByTriggerSpecific(trigger, resolvedType.id, -1);
            if (script) {
                return script;
            }

            if (resolvedType.category !== -1) {
                script = ScriptProvider.getByTriggerSpecific(trigger, -1, resolvedType.category);
                if (script) {
                    return script;
                }
            }
        ...
        let script = ScriptProvider.getByTriggerSpecific(trigger, baseType.id, -1);
        if (script) {
            return script;
        }

        if (baseType.category !== -1) {
            script = ScriptProvider.getByTriggerSpecific(trigger, -1, baseType.category);
            if (script) {
                return script;
            }
        }

        return ScriptProvider.getByTriggerSpecific(trigger, -1, -1) ?? null;
    }

    resolveNpcType(target: Npc): NpcType | null {
        const type = NpcType.get(target.type);
        if (!type) {
            return null;
        }

        const resolvedId = type.getMultiNpcId(id => this.getVarBit(id), id => this.getVar(id) as number);
        if (resolvedId === -1) {
            return null;
        }

        return NpcType.get(resolvedId) ?? null;
    }

    resolveLocType(target: Loc): LocType | null {
        const type = LocType.get(target.type);
        if (!type) {
            return null;
        }

        const resolvedId = type.getMultiLocId(id => this.getVarBit(id), id => this.getVar(id) as number);
        if (resolvedId === -1) {
            return null;
        }
    ...
    defaultOp() {
        ...
        if (!Environment.NODE_PRODUCTION && !opTrigger && !apTrigger) {
            let debugname = '_';
            if (this.target instanceof Npc) {
                const type = this.resolveNpcType(this.target);
            ...
            sendMissingTriggerMessage(this, `No trigger for [${ServerTriggerType[this.targetOp + 7].toLowerCase()},${debugname}]`, true);
        } else {
    ...
    inOperableDistance(target: Entity): boolean {
        ...
        if (target instanceof PathingEntity) {
            ...
            const forceapproach = this.resolveLocType(target)?.forceapproach;
            if (typeof forceapproach === 'undefined') {
                return false;
            }
    ...
    validateTarget(): boolean {
        ...
        if (this.target instanceof Npc) {
            const type = this.resolveNpcType(this.target);
            if (!type || this.targetSubject.type !== type.id) {
                return false;
            }
        }
        ...
        const valid = this.target.isValid(this.hash64);
        return valid;
    }
    ...
    private getFirstHeadiconBit(start: number, end: number): number {
        for (let icon = start; icon <= end; icon++) {
            if ((this.headicons & (0x1 << icon)) !== 0) {
                return icon;
            }
        }
    ...
    private getAppearancePkHeadicon(): number {
        if (this.pkHeadicon !== -1) {
            return this.pkHeadicon;
        }

        return this.getFirstHeadiconBit(0, 1);
    }

    private getAppearancePrayerHeadicon(): number {
        if (this.prayerHeadicon !== -1) {
            return this.prayerHeadicon;
        }

        const icon = this.getFirstHeadiconBit(2, 31);
        return icon === -1 ? -1 : icon - 2;
    }
    ...
    generateAppearance(): Uint8Array {
        ...
        stream.p1(this.getAppearancePkHeadicon());
        stream.p1(this.getAppearancePrayerHeadicon());
        ...
        if (this.transmogNpc !== -1) {
            stream.p2(-1);
            stream.p2(this.transmogNpc);
        } else {
            const skippedSlots = [];
            ...
            for (const obj of this.appearanceSlotOverrides.values()) {
                const config = ObjType.get(obj);
            ...
            for (let slot = 0; slot < APPEARANCE_SLOT_COUNT; slot++) {
                if (skippedSlots.indexOf(slot) !== -1) {
                    stream.p1(0);
                    continue;
                }

                const override = this.appearanceSlotOverrides.get(slot);
                const equip = typeof override === 'undefined' ? worn.get(slot) : { id: override };
                if (!equip) {
    ...
    setAppearanceSlotOverride(slot: number, obj: number): void {
        this.checkAppearanceSlot(slot);
        this.appearanceSlotOverrides.set(slot, obj);
        this.buildAppearance(this.appearanceInv);
    }

    clearAppearanceSlotOverride(slot: number): void {
        this.checkAppearanceSlot(slot);
        this.appearanceSlotOverrides.delete(slot);
        this.buildAppearance(this.appearanceInv);
    }

    private checkAppearanceSlot(slot: number): void {
        if (!Number.isInteger(slot) || slot < 0 || slot >= APPEARANCE_SLOT_COUNT) {
            throw new Error(`Invalid appearance slot override: ${slot}`);
        }
    }
    ...
    getInventory(inv: number): Inventory | null {
        ...
        const viewedBank = getViewedBankInventory(this, inv);
        if (typeof viewedBank !== 'undefined') {
            return viewedBank;
        }
    ...
    setVar(id: number, value: number | string) {
        ...
        if (varp.type === ScriptVarType.STRING && typeof value === 'string') {
            ...
            const previous = this.vars[varp.id];
            ...
            if (previous !== value && LocType.dependsOnVar(id)) {
    ...
    addXp(stat: number, xp: number) {
        ...
        if (this.baseLevels[stat] > before) {
            ...
            if (total === 1980) {
                this.addSessionLog(LoggerEventType.ADVENTURE, 'Reached total level 1980 - you beat p2p!');
            }
            if (freeTotal === 1485) {
    ...
    getNpcKillCount(npcType: number): number {
        return this.npcKillCounts.get(npcType)?.kills ?? this.npcKillCountsPending.get(npcType) ?? 0;
    }
    ...
    playJingle(id: number, delay: number = Midi.getLength(id)): void {
    ...
    isInWilderness(): boolean {
        if (this.level >= 1 && this.level <= 2 && this.x >= 46 * 64 && this.x < 48 * 64 && this.z >= 77 * 64 && this.z < 79 * 64) {
            return false;
        }
        if (this.level === 0 && this.x >= 47 * 64 && this.x < 48 * 64 && this.z >= 75 * 64 && this.z < 76 * 64) {
            return false;
        }
        if (this.level >= 0 && this.level <= 3 && this.x >= 126 * 64 && this.x < 133 * 64 && this.z >= 135 * 64 && this.z < 180 * 64) {
            return true;
        }
        if (this.level === 0 && this.x >= 126 * 64 && this.x < 133 * 64 && this.z >= 235 * 64 && this.z < 242 * 64) {
            return true;
        }
    ...
    lastLoginInfo() {
        if (this.loginModalId !== -1 && this.lastLoginInfoPacket) {
            this.write(this.lastLoginInfoPacket);
            return;
        }
        ...
        this.lastLoginInfoPacket = new LastLoginInfo(lastIp, daysSinceLogin, daysSinceRecoveriesChanged, this.messageCount, warnMembersInNonMembers);
        this.write(this.lastLoginInfoPacket);
    ...
    ifOpenMain(component: number): void {
        this.write(new IfOpenMain(component));
    }

    ifSetText(component: number, text: string): void {
        this.write(new IfSetText(component, text));
    }

    ifSetScrollPos(component: number, y: number): void {
        this.write(new IfSetScrollPos(component, y));
    }
