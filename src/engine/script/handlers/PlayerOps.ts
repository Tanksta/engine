...
import { forEachInstancedMapsquareForCoord, isInstancedMapsquare, tzHaarFightCaveInstanceBaseFromSlot } from '#/engine/TzHaarFightCave.js';
...
function resolveMidiId(state: ScriptState): number {
    // Most of this branch's music content passes midi names as strings.
    // Support both named tracks and pre-resolved numeric ids.
    if (state.ssp > 0) {
        const name = state.popString();
        if (!name.length) {
            return -1;
        }
        return MidiNames.getByName(name);
    }

    return state.popInt();
}

function interfaceScriptOperandCount(opcode: number): number {
    switch (opcode) {
        case InterfaceScriptOp.StatLevel:
        case InterfaceScriptOp.StatBaseLevel:
        case InterfaceScriptOp.StatXp:
        case InterfaceScriptOp.PushVar:
        case InterfaceScriptOp.StatXpRemaining:
        case InterfaceScriptOp.PushVarPercentage:
        case InterfaceScriptOp.PushVarBit:
        case InterfaceScriptOp.PushConstant:
            return 1;
        case InterfaceScriptOp.InvCount:
        case InterfaceScriptOp.InvContains:
        case InterfaceScriptOp.TestBit:
            return 2;
        default:
            return 0;
    }
...
function resendVarp(player: Player, varpId: number, resent: Set<number>): void {
    const varp = VarPlayerType.get(varpId);
    const value = player.getVar(varpId);
    if (!resent.has(varpId) && varp?.transmit && typeof value === 'number') {
        player.setVar(varpId, value);
        resent.add(varpId);
    }
}

function resendIfButtonVarps(player: Player): void {
    const com = Component.get(player.lastCom);
    if (!com?.scripts) {
        return;
    }

    const resent = new Set<number>();
    for (const script of com.scripts) {
        for (let i = 0; i < script.length; i++) {
            ...
            if (opcode === InterfaceScriptOp.PushVar || opcode === InterfaceScriptOp.PushVarPercentage) {
                if (i + 1 >= script.length) {
                    break;
                }
                const varpId = script[i + 1];
                resendVarp(player, varpId, resent);
            } else if (opcode === InterfaceScriptOp.TestBit) {
                if (i + 1 >= script.length) {
                    break;
                }
                const varpId = script[i + 1];
                resendVarp(player, varpId, resent);
            } else if (opcode === InterfaceScriptOp.PushVarBit) {
                if (i + 1 >= script.length) {
                    break;
                }
                const varbitId = script[i + 1];
                const varbit = VarBitType.get(varbitId);
                if (varbit && varbit.basevar !== -1) {
                    resendVarp(player, varbit.basevar, resent);
                }
            }

            i += interfaceScriptOperandCount(opcode);
        }
    }
}

function isPlayerLocZoneVisible(player: Player, position: CoordGrid): boolean {
    if (position.level !== player.level || player.originX < 0 || player.originZ < 0) {
        return false;
    }

    const zoneX = CoordGrid.zone(position.x);
    const zoneZ = CoordGrid.zone(position.z);
    const originZoneX = CoordGrid.zone(player.originX);
    const originZoneZ = CoordGrid.zone(player.originZ);
    if (zoneX < originZoneX - 6 || zoneX > originZoneX + 6 || zoneZ < originZoneZ - 6 || zoneZ > originZoneZ + 6) {
        return false;
    }
...
function writePlayerLocZone(player: Player, position: CoordGrid): boolean {
    if (!isPlayerLocZoneVisible(player, position)) {
        return false;
    }

    const zoneX = CoordGrid.zone(position.x);
    const zoneZ = CoordGrid.zone(position.z);
    player.write(new UpdateZonePartialFollows(zoneX, zoneZ, player.originX, player.originZ));
    return true;
}
...
const PlayerOps: CommandHandlers = {
    ...
    [ScriptOpcode.P_FINDUID]: state => {
        ...
        const isCurrentCombatTarget =
            state.intOperand === 1 &&
            state._activePlayer2?.uid === uid &&
            (state.trigger === ServerTriggerType.OPPLAYER2 || state.trigger === ServerTriggerType.APPLAYER2);

        if (!player || (!isCurrentCombatTarget && !player.canAccess())) {
            if (state.trigger === ServerTriggerType.IF_BUTTON && state.self instanceof Player) {
                resendIfButtonVarps(state.self);
            }
            state.pushInt(0);
            return;
        }
    ...
    [ScriptOpcode.APPEARANCE_SETSLOT]: checkedHandler(ActivePlayer, state => {
        const [slot, obj] = state.popInts(2);

        state.activePlayer.setAppearanceSlotOverride(slot, check(obj, ObjTypeValid).id);
    }),

    [ScriptOpcode.APPEARANCE_CLEARSLOT]: checkedHandler(ActivePlayer, state => {
        state.activePlayer.clearAppearanceSlotOverride(state.popInt());
    }),

    [ScriptOpcode.P_LOCADD]: checkedHandler(ActivePlayer, state => {
        const [coord, loc, angle, shape] = state.popInts(4);

        const position = check(coord, CoordValid);
        const type = check(loc, LocTypeValid);
        const locAngle = check(angle, LocAngleValid);
        const locShape = check(shape, LocShapeValid);

        if (!writePlayerLocZone(state.activePlayer, position)) {
            return;
        }
        state.activePlayer.write(new LocAddChange(CoordGrid.packZoneCoord(position.x, position.z), type.id, locShape, locAngle));
    }),

    [ScriptOpcode.P_LOCDEL]: checkedHandler(ActivePlayer, state => {
        const [coord, angle, shape] = state.popInts(3);
        ...
        state.activePlayer.write(new LocDel(CoordGrid.packZoneCoord(position.x, position.z), locShape, locAngle));
    }),

    [ScriptOpcode.P_LOCRESET]: checkedHandler(ActivePlayer, state => {
        const position = check(state.popInt(), CoordValid);

        if (!isPlayerLocZoneVisible(state.activePlayer, position)) {
            return;
        }
        World.gameMap.getZone(position.x, position.z, position.level).writeFullFollows(state.activePlayer);
    }),
    ...
    [ScriptOpcode.P_STRINGDIALOG]: checkedHandler(ProtectedActivePlayer, state => {
        state.activePlayer.write(new PStringDialog(state.popString()));
    }),
    ...
    [ScriptOpcode.P_TELEJUMP_INSTANCE]: checkedHandler(ProtectedActivePlayer, state => {
        const coord = state.popInt();
        const position: CoordGrid = check(coord, CoordValid);
        const baseLevel = tzHaarFightCaveInstanceBaseFromSlot(state.activePlayer.slot);

        if (isInstancedMapsquare(position.x, position.z)) {
            forEachInstancedMapsquareForCoord(position.x, position.z, (mx, mz) => {
                World.gameMap.ensureInstancedMapsquare(mx, mz, baseLevel);
            });
        }
    ...
    [ScriptOpcode.P_FLYWALK]: checkedHandler(ProtectedActivePlayer, state => {
        const coord: CoordGrid = check(state.popInt(), CoordValid);

        state.activePlayer.queueTemporaryFlyWaypoints(coord.x, coord.z);
    }),
    ...
    [ScriptOpcode.PET_ACQUIRED]: checkedHandler(ActivePlayer, state => {
        const sourceDetail = state.popString();
        const sourceType = state.popString();
        const petObj = check(state.popInt(), ObjTypeValid);

        state.activePlayer.addPetAcquisition(petObj.id, sourceType, sourceDetail);
    }),

    [ScriptOpcode.NPC_KILLCOUNT]: checkedHandler(ActivePlayer, state => {
        const npcType = check(state.popInt(), NpcTypeValid);

        state.pushInt(state.activePlayer.getNpcKillCount(npcType.id));
    }),
    ...
    [ScriptOpcode.TELEBLOCK_SET]: checkedHandler(ActivePlayer, state => {
        const ticks = check(state.popInt(), NumberNotNull);
        const player = state.activePlayer;

        if (ticks <= 0) {
            player.teleblocked = false;
            player.teleblockUntil = -1;
            return;
        }

        player.teleblocked = true;
        player.teleblockUntil = World.currentTick + ticks;
    }),

    [ScriptOpcode.TELEBLOCK_TICKS]: checkedHandler(ActivePlayer, state => {
        const player = state.activePlayer;
        if (!player.teleblocked || player.teleblockUntil <= World.currentTick) {
            player.teleblocked = false;
            player.teleblockUntil = -1;
            state.pushInt(0);
            return;
        ...
        state.pushInt(player.teleblockUntil - World.currentTick);
    }),
    ...
    [ScriptOpcode.MIDI_SONG]: state => {
        const id = resolveMidiId(state);

        const player = state.activePlayer;
        if (player.lowMemory || id === -1) {
            return;
        }
    ...
    [ScriptOpcode.BUSY2]: state => {
        state.pushInt(state.activePlayer.hasInteraction() || state.activePlayer.hasWaypoints() || state.activePlayer.hasTemporaryMoveStrategy() ? 1 : 0);
    },
    ...
    [ScriptOpcode.HEADICONS_SET]: state => {
        ...
        state.activePlayer.masks |= PlayerInfoProt.APPEARANCE;
    },
    ...
    [ScriptOpcode.AFK_EVENT_SKILL]: state => {
        state.pushInt((Environment.NODE_DEBUG || state.activePlayer.staffModLevel < 2) && state.activePlayer.afkSkillingEventReady ? 1 : 0);
        state.activePlayer.afkSkillingEventReady = false;
    },
    ...
    [ScriptOpcode.STAT_SETLEVEL]: checkedHandler(ActivePlayer, state => {
        const [stat, level] = state.popInts(2);
        const validatedStat = check(stat, PlayerStatValid);
        state.activePlayer.setLevel(validatedStat, check(level, NumberNotNull));
        if (validatedStat === PlayerStat.HITPOINTS) {
            state.activePlayer.heroPoints.clear();
        }
    }),

    [ScriptOpcode.STAT_EXPERIENCE]: checkedHandler(ActivePlayer, state => {
        const stat = check(state.popInt(), PlayerStatValid);
        state.pushInt(state.activePlayer.stats[stat]);
    }),

    [ScriptOpcode.STAT_RESTORE_EXACT]: checkedHandler(ActivePlayer, state => {
        const [statValue, experienceValue, currentLevelValue] = state.popInts(3);
        const stat = check(statValue, PlayerStatValid);
        const experience = Math.min(2_000_000_000, Math.max(0, experienceValue));
        const currentLevel = Math.min(255, Math.max(0, currentLevelValue));

        state.activePlayer.stats[stat] = experience;
        state.activePlayer.baseLevels[stat] = getLevelByExp(experience);
        state.activePlayer.levels[stat] = currentLevel;
        state.activePlayer.changeStat(stat);

        const combatLevel = state.activePlayer.getCombatLevel();
        if (state.activePlayer.combatLevel !== combatLevel) {
            state.activePlayer.combatLevel = combatLevel;
    ...
    [ScriptOpcode.SETIDKCOLOUR]: checkedHandler(ActivePlayer, state => {
        const [slot, colour] = state.popInts(2);
        const colorSlot = check(slot, NumberNotNull);
        const color = check(colour, NumberNotNull);
        if (colorSlot < 0 || colorSlot >= state.activePlayer.colors.length) {
            throw new Error(`Invalid idkit color slot: ${colorSlot}`);
        }
    ...
    [ScriptOpcode.P_TRANSMOGRIFY]: checkedHandler(ProtectedActivePlayer, state => {
        const npc = state.popInt();
        if (npc !== -1) {
            check(npc, NpcTypeValid);
        }

        state.activePlayer.transmogNpc = npc;
        state.activePlayer.buildAppearance(state.activePlayer.appearanceInv);
    }),
