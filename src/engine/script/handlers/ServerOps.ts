...
import { resolveTzHaarFightCaveCoord } from '#/engine/TzHaarFightCave.js';
import World from '#/engine/World.js';
import Environment from '#/util/Environment.js';
import Midi from '#/cache/midi/Midi.js';
import { MidiNames } from '#/cache/midi/MidiNames.js';

const RUNEDAY_EPOCH = Date.UTC(2002, 1, 27);
...
const ServerOps: CommandHandlers = {
    ...
    [ScriptOpcode.DATE_MINUTES]: state => {
        state.pushInt(Math.floor(Date.now() / 60000));
    },

    [ScriptOpcode.DATE_RUNEDAY]: state => {
        state.pushInt(Math.floor((Date.now() - RUNEDAY_EPOCH) / DAY_MS));
    },
    ...
    [ScriptOpcode.SPOTANIM_MAP]: state => {
        ...
        const position: CoordGrid = resolveTzHaarFightCaveCoord(state, check(coord, CoordValid));
    ...
    [ScriptOpcode.MAP_BLOCKED]: state => {
        const coord: CoordGrid = resolveTzHaarFightCaveCoord(state, check(state.popInt(), CoordValid));
    ...
    [ScriptOpcode.PROJANIM_PL]: state => {
        ...
        const srcPos: CoordGrid = resolveTzHaarFightCaveCoord(state, check(srcCoord, CoordValid));
        const spotanimType: SpotanimType = check(spotanim, SpotAnimTypeValid);
    ...
    [ScriptOpcode.PROJANIM_MAP]: state => {
        ...
        const spotanimType: SpotanimType = check(spotanim, SpotAnimTypeValid);
        const srcPos: CoordGrid = resolveTzHaarFightCaveCoord(state, check(srcCoord, CoordValid));
        const dstPos: CoordGrid = resolveTzHaarFightCaveCoord(state, check(dstCoord, CoordValid));
    ...
    [ScriptOpcode.MAP_FINDSQUARE]: state => {
        ...
        const origin: CoordGrid = resolveTzHaarFightCaveCoord(state, check(coord, CoordValid));
    ...
    [ScriptOpcode.MIDI_LENGTH]: state => {
        const track = resolveMidiId(state);
        if (track === -1) {
            state.pushInt(0);
            return;
        }
