...
export default class FileStream {
    ...
    read(archive: number, file: number, decompress: boolean = false): Uint8Array | null {
        ...
        if (archive === 0) {
            ...
            const isGzip = data.data.length >= 2 && data.data[0] === 0x1f && data.data[1] === 0x8b;
            if (!isGzip) {
                return data.data;
            }
    ...
    write(archive: number, file: number, data: Uint8Array, version: number = 0): boolean {
        ...
        // Keep the in-memory packed cache aligned with what we just wrote so
        // later reads in the same process don't return stale bytes.
        if (!this.discardPacked) {
