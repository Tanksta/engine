...
export default class Packet extends DoublyLinkable {
    ...
    g1b_alt1(): number {
        return (this.view.getUint8(this.pos++) - 128) << 24 >> 24;
    }

    g1b_alt2(): number {
        return (-this.view.getUint8(this.pos++)) << 24 >> 24;
    }
    ...
    ig2b_alt2(): number {
        const result: number = this.ig2_alt2();
    ...
    g4_alt3(): number {
        this.pos += 4;
        return (this.data[this.pos - 2] << 24) |
            (this.data[this.pos - 1] << 16) |
            (this.data[this.pos - 4] << 8) |
