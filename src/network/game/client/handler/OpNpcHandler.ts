...
export function resolveNpcTypeForPlayer(player: NetworkPlayer, npc: { type: number }): NpcType | null {
    const type = NpcType.get(npc.type);
    if (!type) {
        return null;
    }

    const resolvedId = type.getMultiNpcId(id => player.getVarBit(id), id => player.getVar(id) as number);
    if (resolvedId === -1) {
        return null;
    }
...
export default class OpNpcHandler extends ClientGameMessageHandler<OpNpc> {
    handle(message: OpNpc, player: NetworkPlayer): boolean {
        ...
        const npcType = resolveNpcTypeForPlayer(player, npc);
        if (!npcType || !npcType.op || npcType.op[message.op - 1] === null || npcType.op[message.op - 1] === 'hidden') {
        ...
        player.targetSubject.type = npcType.id;
