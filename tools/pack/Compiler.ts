...
export class CompilerTypeInfo {
    ...
    private nextKey(id: number): string {
...
type MissingConfigFallback = {
    readonly kind: string;
    readonly info: CompilerTypeInfo;
    readonly id: number;
    readonly missing: MissingConfigSymbol[];
    readonly reason: string;
    readonly key?: string;
};

function useMissingConfigFallback(fallback: MissingConfigFallback) {
    const key = fallback.key ?? fallback.id.toString();

    fallback.missing.push({
        kind: fallback.kind,
        id: fallback.id,
        name: fallback.info.map[key] ?? '<unknown>',
        reason: fallback.reason
    });

    fallback.info.vartype[key] = ScriptVarType.getType(ScriptVarType.INT);
    fallback.info.protect[key] = true;
}

function writeMissingConfigSymbolReport(missing: MissingConfigSymbol[]) {
    const reportDir = 'reports/revision-migration';
    fs.mkdirSync(reportDir, { recursive: true });
    fs.writeFileSync(
        `${reportDir}/unresolved-script-symbols.json`,
        `${JSON.stringify(
            {
...
export function runServerCompiler() {
    ...
    const missingConfigSymbols: MissingConfigSymbol[] = [];
    ...
    for (const [key, name] of Object.entries(componentInfo.map)) {
        ...
        if (!com) {
            useMissingConfigFallback({
                kind: 'component',
                info: componentInfo,
                id,
                missing: missingConfigSymbols,
                reason: 'No component entry exists in the active interface archive.',
                key
            });
    ...
    for (let id = 0; id <= varpInfo.max; id++) {
        ...
        if (!varp) {
            useMissingConfigFallback({
                ...
                id,
                missing: missingConfigSymbols,
                reason: 'No varp entry exists in the active cache config.'
            });
            continue;
        }
    ...
    for (let id = 0; id <= varbitInfo.max; id++) {
        ...
        if (!varbit) {
            useMissingConfigFallback({
                ...
                id,
                missing: missingConfigSymbols,
                reason: 'No varbit entry exists in the active cache config.'
            });
            continue;
        }
        ...
        if (!basevar) {
            useMissingConfigFallback({
                ...
                id,
                missing: missingConfigSymbols,
                reason: `Base varp ${varbit.basevar} is absent from the active cache config.`
            });
            continue;
        }
    ...
    for (let id = 0; id <= paramInfo.max; id++) {
        ...
        if (!param) {
            useMissingConfigFallback({
                ...
                id,
                missing: missingConfigSymbols,
                reason: 'No param entry exists in the active server config.'
            });
            continue;
        }
    ...
    writeMissingConfigSymbolReport(missingConfigSymbols);
