...
const TRANSPARENT_RGB = 0xff00ff;

function getPaletteRgb(img: { bitmap: Bitmap }, pos: number): number {
    if (img.bitmap.data[pos + 3] === 0) {
...
export function generatePixelOrder(img: { bitmap: Bitmap }) {
    ...
    for (let j = 0; j < img.bitmap.width * img.bitmap.height; j += 4) {
        const pos = j * 4;
        const current = getPaletteRgb(img, pos);
...
export function writeImage(img: { bitmap: Bitmap }, data: Packet, index: Packet, colors: number[], meta: Sprite | null = null) {
    ...
    if (pixelOrder === 0) {
        for (let j = 0; j < img.bitmap.width * img.bitmap.height; j++) {
            ...
            const rgb = getPaletteRgb(img, pos);
...
export async function convertImage(index: Packet, srcPath: string, safeName: string) {
    ...
    if (sprites.length > 1) {
        const tilesPerRow = Math.floor(img.bitmap.width / tileX);
        ...
        for (let i = 0; i < sprites.length; i++) {
            ...
            writeImage(tile, data, index, colors, sprites[i]);
        }
    } else {
...
export async function convertFontImage(index: Packet, srcPath: string, safeName: string, columns: number = 16, rows: number = 16) {
    const data = Packet.alloc(4);
    data.p2(index.pos);

    const img = await Jimp.read(`${srcPath}/${safeName}.png`);

    if (img.bitmap.width % columns !== 0 || img.bitmap.height % rows !== 0) {
        throw new Error(`FONT ${safeName} must be divisible into a ${columns}x${rows} grid.`);
    }

    const tileWidth = img.bitmap.width / columns;
    const tileHeight = img.bitmap.height / rows;
    ...
    const pixelOrder = generatePixelOrder(img);
