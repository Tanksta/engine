...
import { lookupParamDefinition, lookupParamValue } from '#tools/pack/config/ParamConfig.js';
...
export function parseLocConfig(key: string, value: string): ConfigValue | null | undefined {
    const isModelKey = /^model\d*$/.test(key) || /^ldmodel\d*$/.test(key);
    // prettier-ignore
    const stringKeys = [
        'name', 'desc',
        'op1', 'op2', 'op3', 'op4', 'op5',
    ...
    if (stringKeys.includes(key) || isModelKey) {
        ...
        // recolours are model-space (HSL16) values, matched verbatim against faceColour
        ...
        const param = lookupParamDefinition(paramKey);
        if (!param) {
            return null;
        }
        ...
        if (config) {
            ...
            for (let j = 0; j < config.length; j++) {
                ...
                if (key === 'name') {
                    ...
                    const separator = modelValue.lastIndexOf(',');
                    if (separator === -1) {
                        srcModels.push(modelValue);
                    } else {
                        const modelName = modelValue.substring(0, separator);
                        const shape = parseInt(modelValue.substring(separator + 1));
                        const modelId = ModelPack.getByName(modelName);
                        if (modelId === -1 || Number.isNaN(shape) || shape < 0 || shape > 22) {
                            throw packStepError(debugname, `Invalid loc model: ${modelValue}`);
                        }

                        modelFlags[modelId] |= 0x4;
                        explicitModels.push({ model: modelId, shape });
                    }
                } else if (key.startsWith('recol')) {
                    ...
                    multivar = value as string;
                } else if (key === 'multiloc') {
                    const [stateRaw, locName] = (value as string).split(',', 2);
                    const state = parseInt(stateRaw);
                    if (Number.isNaN(state) || state < 0 || !locName) {
                        throw packStepError(debugname, `Invalid multiloc value: ${value}`);
                    }

                    const locId = locName === 'null' ? -1 : LocPack.getByName(locName);
                    if (locId === -1 && locName !== 'null') {
                        throw packStepError(debugname, `Invalid multiloc target: ${locName}`);
                    }

                    multiloc[state] = locId;
                } else if (key === 'param') {
            ...
            if (multivar !== null || multiloc.length > 0) {
                if (multivar !== null && multiloc.length > 0) {
                    const varbit = VarbitPack.getByName(multivar);
                    const varp = varbit === -1 ? VarpPack.getByName(multivar) : -1;
                    if (varbit !== -1 || varp !== -1) {
                        client.p1(77);
                        client.p2(varbit === -1 ? 65535 : varbit);
                        client.p2(varp === -1 ? 65535 : varp);
                        client.p1(multiloc.length - 1);

                        for (let index = 0; index < multiloc.length; index++) {
                            const locId = multiloc[index];
                            client.p2(locId === undefined || locId === -1 ? 65535 : locId);
                        }
                    }
                }
            }
            ...
            const models: LocModelShape[] = [...explicitModels];
