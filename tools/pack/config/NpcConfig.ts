...
import { CategoryPack, HuntPack, ModelPack, NpcPack, SeqPack, TexturePack, VarbitPack, VarpPack } from '#tools/pack/PackFile.js';
...
export function parseNpcConfig(key: string, value: string): ConfigValue | null | undefined {
    ...
    // prettier-ignore
    const booleanKeys = [
        'hasalpha', 'minimap', 'members', 'givechase', 'alwaysontop', 'active'
    ];
        ...
        if (config) {
            ...
            for (let j = 0; j < config.length; j++) {
                ...
                if (key === 'name') {
                    ...
                    modelFlags[value as number] |= 0x2; // todo: use context from script compiler
                } else if (key.startsWith('recol') || key.startsWith('retex')) {
                    ...
                    const [indexText, npcName] = (value as string).split(',');
                    const index = parseInt(indexText);
                    const npcId = NpcPack.getByName(npcName);
                    if (Number.isNaN(index) || npcId === -1) {
                        throw new Error(`Unknown multinpc: ${value}`);
                    }
                    multinpc[index] = npcId;
                } else if (key === 'active') {
                    if (value === false) {
                        client.p1(107);
                        active = false;
            ...
            if (multivarp !== -1 || multivarbit !== -1) {
                if (multinpc.length > 0) {
                    client.p1(106);
                    client.p2(multivarbit !== -1 ? multivarbit : 65535);
                    client.p2(multivarp !== -1 ? multivarp : 65535);
                    client.p1(multinpc.length - 1);

                    for (let k = 0; k < multinpc.length; k++) {
                        client.p2(typeof multinpc[k] !== 'undefined' ? multinpc[k] : 65535);
                    }
                }
            }
