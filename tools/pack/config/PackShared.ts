...
function shouldBuildServerConfig(ext: string, out: string): boolean {
    return shouldBuild(`${Environment.BUILD_SRC_DIR}/scripts`, ext, out) || shouldBuild('tools/pack/config', '.ts', out);
}

function getNativeVarMetadataTree(extension: string): Set<string> | null {
    if (extension !== '.varp' && extension !== '.varbit') {
        return null;
    }

    const source = `${Environment.BUILD_SRC_DIR}/scripts/_unpack/349/all${extension}`;
    ...
    if (extension === '.varp') {
        const supplements = [
            `${Environment.BUILD_SRC_DIR}/scripts/skill_magic/configs/spellbook.varp`,
            `${Environment.BUILD_SRC_DIR}/scripts/skill_combat/configs/autocast.varp`
        ];
...
export async function packConfigs(cache: FileStream, modelFlags: number[]) {
    ...
    if (shouldBuildServerConfig('.param', 'data/pack/server/param.dat')) {
        await readConfigs(
            dirTree,
            '.param',
            ['type'],
    ...
    const jag = fs.existsSync('data/pack/client/config') ? Jagfile.load('data/pack/client/config') : Jagfile.new();
    ...
    if (shouldBuildFile(`${Environment.BUILD_SRC_DIR}/pack/category.pack`, 'data/pack/server/category.dat') || shouldBuild('tools/pack/config', '.ts', 'data/pack/server/category.dat')) {
        ...
        for (let i = 0; i < CategoryPack.size; i++) {
            length += CategoryPack.getById(i).length + 3;
        }

        const dat = Packet.alloc(length);
    ...
    if (
        shouldBuildServerConfig('.dbrow', 'data/pack/server/dbrow.dat') ||
        shouldBuildServerConfig('.dbtable', 'data/pack/server/dbtable.dat')
    ) {
    ...
    if (shouldBuildServerConfig('.enum', 'data/pack/server/enum.dat')) {
    ...
    if (shouldBuildServerConfig('.inv', 'data/pack/server/inv.dat')) {
    ...
    if (shouldBuildServerConfig('.mesanim', 'data/pack/server/mesanim.dat')) {
    ...
    if (shouldBuildServerConfig('.struct', 'data/pack/server/struct.dat')) {
    ...
    if (shouldBuildServerConfig('.seq', 'data/pack/server/seq.dat') || !jag.has('seq.dat') || !jag.has('seq.idx')) {
        await readConfigs(
            dirTree,
            '.seq',
            [],
            ...
            (client: Packet, _server: Packet): boolean => {
                return Packet.checkcrc(client.data, 0, client.pos, -1750288226);
            }
        );
    }

    if (shouldBuildServerConfig('.loc', 'data/pack/server/loc.dat') || !jag.has('loc.dat') || !jag.has('loc.idx')) {
        await readConfigs(
            dirTree,
            '.loc',
            [],
            ...
            (client: Packet, _server: Packet): boolean => {
                return Packet.checkcrc(client.data, 0, client.pos, -2115590254);
            }
        );
    }

    if (shouldBuildServerConfig('.flo', 'data/pack/server/flo.dat') || !jag.has('flo.dat') || !jag.has('flo.idx')) {
        await readConfigs(
            dirTree,
            '.flo',
            [],
            ...
            (client: Packet, _server: Packet): boolean => {
                return Packet.checkcrc(client.data, 0, client.pos, -71030816);
            }
        );
    }

    if (shouldBuildServerConfig('.spotanim', 'data/pack/server/spotanim.dat') || !jag.has('spotanim.dat') || !jag.has('spotanim.idx')) {
        await readConfigs(
            dirTree,
            '.spotanim',
            [],
            ...
            (client: Packet, _server: Packet): boolean => {
                return Packet.checkcrc(client.data, 0, client.pos, -451564820);
            }
        );
    }

    if (shouldBuildServerConfig('.npc', 'data/pack/server/npc.dat') || !jag.has('npc.dat') || !jag.has('npc.idx')) {
        await readConfigs(
            dirTree,
            '.npc',
            [],
            ...
            (client: Packet, _server: Packet): boolean => {
                return Packet.checkcrc(client.data, 0, client.pos, -1218203581);
            }
        );
    }

    if (shouldBuildServerConfig('.obj', 'data/pack/server/obj.dat') || !jag.has('obj.dat') || !jag.has('obj.idx')) {
        await readConfigs(
            dirTree,
            '.obj',
            [],
            ...
            (client: Packet, _server: Packet): boolean => {
                return Packet.checkcrc(client.data, 0, client.pos, 577509193);
            }
        );
    }

    if (shouldBuildServerConfig('.idk', 'data/pack/server/idk.dat') || !jag.has('idk.dat') || !jag.has('idk.idx')) {
        await readConfigs(
            dirTree,
            '.idk',
            [],
    ...
    const varpMetadataTree = getNativeVarMetadataTree('.varp');
    if (shouldBuildServerConfig('.varp', 'data/pack/server/varp.dat') || !jag.has('varp.dat') || !jag.has('varp.idx')) {
        await readConfigs(
            varpMetadataTree ?? dirTree,
            '.varp',
            [],
            modelFlags,
            parseVarpConfig,
            ...
            (client: Packet, _server: Packet): boolean => {
                return Packet.checkcrc(client.data, 0, client.pos, 454339809);
            }
        );
    }

    const varbitMetadataTree = getNativeVarMetadataTree('.varbit');
    if (shouldBuildServerConfig('.varbit', 'data/pack/server/varbit.dat') || !jag.has('varbit.dat') || !jag.has('varbit.idx')) {
        await readConfigs(
            varbitMetadataTree ?? dirTree,
            '.varbit',
            ...
            (client: Packet, _server: Packet): boolean => {
                return Packet.checkcrc(client.data, 0, client.pos, 1526193797);
            }
        );
    }

    if (shouldBuildServerConfig('.hunt', 'data/pack/server/hunt.dat')) {
    ...
    if (shouldBuildServerConfig('.varn', 'data/pack/server/varn.dat')) {
    ...
    if (shouldBuildServerConfig('.vars', 'data/pack/server/vars.dat')) {
