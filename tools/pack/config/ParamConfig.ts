...
import { readConfigs } from '#tools/pack/Parse.js';
...
import { getParamDefaultFallback, writeUnresolvedParamDefaults, type UnresolvedParamDefault } from '#tools/pack/config/ConfigFallback.js';
...
let sourceParamTypes: Map<string, number> | null = null;

function getSourceParamTypes(): Map<string, number> {
    if (sourceParamTypes) {
        return sourceParamTypes;
    }

    sourceParamTypes = new Map();
    const configs = readConfigs('.param');
    for (const [debugname, lines] of configs) {
        for (const line of lines) {
            if (!line.startsWith('type=')) {
                continue;
            }

            const type = ScriptVarType.getTypeChar(line.substring('type='.length));
            if (type !== null) {
                sourceParamTypes.set(debugname, type);
            }
            break;
        }
    }
...
export function lookupParamDefinition(name: string): { id: number; type: number } | null {
    const existing = ParamType.getByName(name);
    if (existing) {
        return { id: existing.id, type: existing.type };
    }

    const id = ParamPack.getByName(name);
    if (id === -1) {
        return null;
    }

    const type = getSourceParamTypes().get(name);
...
export function packParamConfigs(configs: Map<string, ConfigLine[]>): { client: PackedData; server: PackedData } {
    ...
    const unresolvedDefaults: UnresolvedParamDefault[] = [];
        ...
        if (config) {
            ...
            for (let j = 0; j < config.length; j++) {
                ...
                if (key === 'type') {
                    ...
                    let paramValue = lookupParamValue(type, defaultValue);
                    if (paramValue === null) {
                        const fallback = getParamDefaultFallback(type);
                        if (fallback === null) {
                        ...
                        unresolvedDefaults.push({
                            param: debugname,
                            type: ScriptVarType.getType(type),
                            value: defaultValue,
                            fallback
                        });
                        paramValue = fallback;
    ...
    writeUnresolvedParamDefaults(unresolvedDefaults);
