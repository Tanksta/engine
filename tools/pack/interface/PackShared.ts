...
const NATIVE_INTERFACE_DIR = `${Environment.BUILD_SRC_DIR}/scripts/_unpack/349/interfaces`;
const NATIVE_INTERFACE_ORDER = `${NATIVE_INTERFACE_DIR}/interface.order`;
const NATIVE_COMPONENT_PATTERN = /^component_(\d+)$/;
...
function parseNativeComponentId(name: string): number | null {
    const match = NATIVE_COMPONENT_PATTERN.exec(name);
    if (!match) {
        return null;
    }
...
function resolveInterfaceId(name: string): number {
    return parseNativeComponentId(name) ?? InterfacePack.getByName(name);
}

function resolvePackId(pack: { getByName(name: string): number }, prefix: string, name: string): number {
    const id = pack.getByName(name);
    if (id !== -1) {
        return id;
    }
    ...
    return Number.isSafeInteger(fallback) ? fallback : -1;
}

function addComponent(component: Record<number, Component>, interfaceOrder: number[], id: number): void {
    if (id < 0 || typeof component[id] !== 'undefined') {
        return;
    }
    ...
    interfaceOrder.push(id);
}

function resetComponent(component: Record<number, Component>, id: number): void {
    component[id] = {
        root: null,
        children: [],
        src: {}
...
function resolveComponentId(lookup: ComponentLookup, nativeComponentOrder: ReadonlyMap<number, readonly number[]>): number {
    const nativeId = parseNativeComponentId(lookup.componentName);
    if (nativeId !== null) {
        return nativeId;
    }

    const exactId = InterfacePack.getByName(`${lookup.interfaceName}:${lookup.componentName}`);
    if (exactId !== -1) {
        return exactId;
    }

    const orderedId = InterfacePack.getByName(`${lookup.interfaceName}:com_${lookup.componentIndex}`);
    if (orderedId !== -1) {
        InterfacePack.register(orderedId, `${lookup.interfaceName}:${lookup.componentName}`);
        return orderedId;
    }

    const ifId = resolveInterfaceId(lookup.interfaceName);
    const nativeOrderId = nativeComponentOrder.get(ifId)?.[lookup.componentIndex];
    if (typeof nativeOrderId !== 'undefined') {
        InterfacePack.register(nativeOrderId, `${lookup.interfaceName}:${lookup.componentName}`);
        return nativeOrderId;
    }

    const inferredId = ifId + lookup.componentIndex + 1;
    if (ifId !== -1 && InterfacePack.getById(inferredId) === '') {
        InterfacePack.register(inferredId, `${lookup.interfaceName}:${lookup.componentName}`);
        return inferredId;
    }
...
function getInterfaceOrder(roots?: ReadonlySet<string>): number[] {
    if (!roots) {
        const order = fs.existsSync(NATIVE_INTERFACE_ORDER) ? loadOrder(NATIVE_INTERFACE_ORDER) : [];
        const seen = new Set(order);

        for (const id of loadOrder(`${Environment.BUILD_SRC_DIR}/pack/interface.order`)) {
            if (seen.has(id)) {
                continue;
            }
        ...
        return loadOrder(`${Environment.BUILD_SRC_DIR}/pack/interface.order`);
    }

    const order: number[] = [];
    for (let id = 0; id < InterfacePack.max; id++) {
        const name = InterfacePack.getById(id);
        if (!name) {
            continue;
        }
...
function resolveLayerId(ifName: string, name: string): number {
    return parseNativeComponentId(name) ?? InterfacePack.getByName(`${ifName}:${name}`);
}

function writeServerAliases(server: Packet): void {
    const aliases = Array.from(InterfacePack.nameToId.entries()).sort((left, right) => left[1] - right[1] || left[0].localeCompare(right[0]));
    server.p2(aliases.length);
...
function countSections(src: readonly string[]): number {
    return src.filter(line => line.startsWith('[')).length;
}

export function packInterface(modelFlags: number[], roots?: ReadonlySet<string>) {
    const component: Record<number, Component> = {};
    const nativeComponentOrder = new Map<number, number[]>();

    const interfaceOrder = getInterfaceOrder(roots);
    for (let i = 0; i < interfaceOrder.length; i++) {
        resetComponent(component, interfaceOrder[i]);
    }

    const loadSource = (src: string[], file: string, native: boolean) => {
        const ifName = file.replace('.if', '');
        if (roots && !roots.has(ifName)) {
            return;
        }

        const ifId = resolveInterfaceId(ifName);
        if (ifId === -1 && !roots) {
            return;
        }

        if (!component[ifId]) {
            addComponent(component, interfaceOrder, ifId);
        }
        ...
        if (!native && !roots && component[ifId].root !== null) {
            return;
        }

        const nativeOrder = nativeComponentOrder.get(ifId);
        if (!native && !roots && nativeOrder && countSections(src) > nativeOrder.length) {
            return;
        }

        // Children contributed by a different interface via "layer=" (e.g. message11
        // attaching the message-of-the-week banner to the welcome screen) must survive
        // this file's reset, and are re-appended last so they draw on top.
        const foreignChildren = component[ifId].children.filter(child => {
            const root = component[child]?.root;
            return typeof root === 'string' && root !== ifName;
        });

        if (!native) {
            resetComponent(component, ifId);
        }
        ...
        for (let i = 0; i < src.length; i++) {
            ...
            if (line.startsWith('[')) {
                ...
                comId = resolveComponentId({
                    interfaceName: ifName,
                    componentName: comName,
                    componentIndex: comIndex++
                }, nativeComponentOrder);
                addComponent(component, interfaceOrder, comId);
                if (comId === -1 || typeof component[comId] === 'undefined') {
                    throw new Error(`Missing component ID ${ifName}:${comName} in ${Environment.BUILD_SRC_DIR}/pack/interface.pack`);
                }

                if (!native) {
                    resetComponent(component, comId);
                }

                component[comId].root = ifName;
                component[ifId].children.push(comId);
                sourceOrder.push(comId);
                continue;
            }
            ...
            if (key === 'layer') {
                const layerId = resolveLayerId(ifName, value);
                if (layerId === -1) {
                    throw new Error(`ERROR: Layer ${ifName}:${value} does not exist`);
                }

                if (native && component[layerId].children.indexOf(comId) !== -1) {
        ...
        for (const child of foreignChildren) {
            if (component[ifId].children.indexOf(child) === -1) {
                component[ifId].children.push(child);
            }
        }

        if (native) {
            nativeComponentOrder.set(ifId, sourceOrder);
        }
    };

    if (!roots && fs.existsSync(NATIVE_INTERFACE_DIR)) {
        loadDir(NATIVE_INTERFACE_DIR, '.if', (src, file) => loadSource(src, file, true));
    }

    loadDir(`${Environment.BUILD_SRC_DIR}/scripts`, '.if', (src, file, path) => {
        if (!roots && path.replace(/\\/g, '/').includes('/_unpack/')) {
            return;
        }

        loadSource(src, file, false);
    });
    ...
    for (let i = 0; i < interfaceOrder.length; i++) {
        ...
        if (com.root === null || lastRoot !== com.root) {
            ...
            if (com.root) {
                client.p2(resolveInterfaceId(com.root));
                lastRoot = com.root;
            } else {
                client.p2(id);
        ...
        if (src.overlayer) {
            const layerId = resolveLayerId(com.root ?? '', src.overlayer as string);
                ...
                if (op) {
                    ...
                    switch (parts[0]) {
                        ...
                        case 'stat_xp':
                            ...
                            break;
                        case 'inv_count': {
                            const comLink = resolveInterfaceId(parts[1]);
                            if (comLink === -1) {
                                printError(`${com.root} invalid lookup ${parts[1]}`);
                            }

                            const objLink = resolvePackId(ObjPack, 'obj', parts[2]);
                            if (objLink === -1) {
                        ...
                        case 'pushvar': {
                            const varpLink = resolvePackId(VarpPack, 'varp', parts[1]);
                            if (varpLink === -1) {
                                printError(`${com.root} invalid lookup ${parts[1]}`);
                            }
                        ...
                        case 'push_varbit': {
                            const varbitLink = resolvePackId(VarbitPack, 'varbit', parts[1]);
                            if (varbitLink === -1) {
                                printError(`${com.root} invalid lookup ${parts[1]}`);
                            }
        ...
        if (comType === 6) {
            if (src.model) {
                const modelId = resolvePackId(ModelPack, 'model', src.model as string);
                if (modelId === -1) {
            ...
            if (src.activemodel) {
                const modelId = resolvePackId(ModelPack, 'model', src.activemodel as string);
                if (modelId === -1) {
            ...
            if (src.anim) {
                const seqId = resolvePackId(SeqPack, 'seq', src.anim as string);
                if (seqId === -1) {
            ...
            if (src.activeanim) {
                const seqId = resolvePackId(SeqPack, 'seq', src.activeanim as string);
                if (seqId === -1) {
