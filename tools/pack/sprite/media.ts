...
export async function packClientMedia(cache: FileStream) {
    ...
    sprites.sort((a, b) => {
        const aExists = fileExists(`${Environment.BUILD_SRC_DIR}/sprites/meta/${basename(a, extname(a))}.opt`);
        const bExists = fileExists(`${Environment.BUILD_SRC_DIR}/sprites/meta/${basename(b, extname(b))}.opt`);
        return aExists === bExists ? 0 : (aExists && !bExists ? 1 : -1);
    });

    const all = new Map<string, Packet>();
    for (const name of sprites) {
        const safeName = basename(name, extname(name));
