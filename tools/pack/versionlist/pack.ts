...
export function shouldRebuildVersionListPack(): boolean {
    return (
...
export function loadCachedModelFlags(modelFlags: number[]): boolean {
...
export function packClientVersionList(cache: FileStream, modelFlags: number[]): boolean {
    ...
    const animFiles = new Map(
        listFilesExt(`${Environment.BUILD_SRC_DIR}/models`, '.anim').map(file => [path.basename(file, '.anim'), file])
    );
    for (let id = 0; id < AnimSetPack.max; id++) {
        ...
        if (data) {
            ...
            const file = animFiles.get(AnimSetPack.getById(id));
            if (file) {
                const anim = new Packet(fs.readFileSync(file));
    ...
    for (let id = 0; id < MidiPack.max; id++) {
        ...
        if (data) {
            ...
            midiIndex.pbool(
                fileExists(`${Environment.BUILD_SRC_DIR}/jingles/${MidiPack.getById(id)}.mid`) ||
                    fileExists(`${Environment.BUILD_SRC_DIR}/songs/${MidiPack.getById(id)}.mid`)
            );
        } else {
            midiVersion.p2(0);
            midiCrc.p4(0);
