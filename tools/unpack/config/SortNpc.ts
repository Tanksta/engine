...
function orderClientProperties(properties: readonly string[]): string[] {
    ...
    const names = ordered.filter(line => line.startsWith('name='));
    const desc = ordered.findIndex(line => line.startsWith('desc='));
    if (names.length > 0 && desc !== -1) {
...
export function sortNpcLines(lines: readonly string[]): string {
        ...
        configs
            .map(([header, ...properties]) => {
                if (typeof header === 'undefined') {
                    throw new Error('NPC config is missing a header');
                }

                const client: string[] = [];
                const server: Array<{ readonly line: string; readonly opcode: number; readonly index: number }> = [];
                ...
                server.sort((left, right) => left.opcode - right.opcode || left.index - right.index);
...
export function sortNpcFiles(): void {
    const files = listFilesExt(`${Environment.BUILD_SRC_DIR}/scripts`, '.npc');
